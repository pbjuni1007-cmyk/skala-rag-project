...
flowchart LR
    ...
    O --> M["일반 코드: Markdown·PDF 보고서·검수표 렌더링"]
```
...
검증을 통과한 결과는 Markdown·PDF 보고서와 검수표로 저장한다. 팀은 검수표를 통해 원문 근거와 작성된 주장의 의미가 일치하는지 확인한 뒤 최종 결과를 승인한다.
...
flowchart TD
    ...
    V -->|검증 실패| F
    V -->|검증 통과| O["Markdown·PDF 보고서·인용 및 공백 검수표 생성"]
...
| 저장·자동 종료 | 검증을 통과한 Markdown·PDF 보고서와 인용·공백 검수표를 저장한다. | 저장에 성공하면 상태를 `human_review_pending`으로 전환한다. 저장에 실패하면 `incomplete`로 종료한다. |
| 팀 검수·제출 판단 | 팀이 원문과 주장의 의미, 적용 조건, 실험 조건을 대조하고 발견된 오류와 미확인 사항의 표현을 검토한 뒤 최종 결과를 승인한다. | 필요한 내용을 수정한 뒤 다시 검수하며, 자동 생성한 PDF의 배치까지 검수하고 팀 승인이 완료된 보고서만 제출한다. |
...
### 4.3 재검색·재평가의 판단과 상충 추적

| 발생 조건 | 다음 행동 | 범위와 종료 조건 |
| --- | --- | --- |
| 기술 조사에서 핵심 근거 부족 | 부족한 facet과 이전 검색 결과를 전달해 질의 수정 | 네 질문을 유지하고 재검색 1회. 다시 부족하면 중간 결과를 남기고 종료 |
| 기술 평가의 구조·인용 검증 실패 | 실패 이유를 반영해 질의 수정 및 평가 재시도 | 같은 1회 재검색 한도를 사용. 근거 부족과 검증 실패를 검색 기록에서 구분 |
| 후속 관점의 unknown 주장 또는 insufficient 상태 | 부족한 항목을 겨냥해 이미 수집한 공식 자료의 확장 영역 검색 | 부족 facet만 재평가 1회. 충분한 주장은 그대로 보존하고 미해소 항목은 unknown으로 유지 |
| 세 관점 평가 완료 | 전체 주장·근거·조건·상충·공백을 join에서 통합 | 실패한 분기가 있으면 종합으로 진행하지 않음. unknown 자체는 종료 사유가 아님 |
| 종합의 형식·배치·인용 오류 | 기존 수정 규칙에 따라 해당 항목 보정 | 수정 후 전체 검증. 상한 소진 시 incomplete |

기술 조사 결과는 세 관점이 공유하지만, 병렬 관점끼리 서로의 결과를 보고 재호출하는 순환은 두지 않았다. 공통 원문과 조건을 먼저 고정하고, 각 관점 내부에서 필요한 부분만 보완한 뒤 한 번 합류한다. 관점 간 판단 차이는 종합과 상충 검수에서 함께 검토한다.

상충은 원문 문자열과 함께 고유 ID, 소유 관점, 관련 주장 후보, 원문에 명시된 주장 ID, 조건과 근거 ID를 보관한다. 후보 연결은 의미 관계가 확정된 연결과 구분한다. 자동 생성 시에는 미분류·미해소로 두며, 원문·조건을 검수해 실제 모순, 조건 차이, 이익과 부담의 절충 중 하나로 분류한다. 해소로 변경하려면 검수자·판단 이유·근거가 있는 검증 주장 두 개 이상이 필요하다. 자동 검사는 원문의 누락·변조, 잘못된 ID, 중복, 검수 요건을 충족하지 않는 해소 처리를 거부한다.

각 실행은 `conflict_records.json`과 `conflict_review.md`를 함께 생성한다. 최종 본문에서 상충을 설명하고 검수표에서 연결 주장과 조건을 확인할 수 있다. 의견이 다르다는 이유만으로 하나를 삭제하거나, 실험 조건이 다른 수치를 평균해 결론을 만들지 않는다.
...
| `joined` | dict | 합류 단계: 세 분기의 주장, 근거, 상충 관계, 자료 공백을 하나의 구조로 통합한다. 상충 원문과 구조화된 conflict_records를 함께 보존한다. |
...
| `output_paths` | dict | 출력 단계: Markdown·PDF 보고서와 인용·공백 검수표 등 생성된 파일의 경로를 저장한다. |
...
### 5.4 운영 설정의 재검증과 질의 재작성 효과

모델 버전·청크·검색 개수·문맥 예산·원문·질문·검색 코드가 달라지면 고정 회귀 질문을 다시 평가한다. 현재 운영 검색 및 문맥 구성 함수를 사용하며, 평가 결과와 설정의 해시가 일치하는 경우에만 기존 통과 결과를 재사용한다. 인덱스 생성 후 유료 모델 호출에 앞서 이 검사를 수행한다. 기준이 없거나 원문·정답이 바뀌어 비교할 수 없으면 중단하고 기준 재검토를 요구한다. 질문별 원문 완전 회수와 조건 회수가 기준보다 낮아져도 진행하지 않는다.

2026-09-22에는 이미 공개된 영어 질문 24개로 검색 개수를 비교했다. 이는 기존 선택·검증 질문을 회귀용으로 재사용한 측정이며 새로운 독립 검증은 아니다. 단일 질문에 운영 문맥 구성 함수를 적용했으므로 네 질문을 합치는 실제 기술 조사 전체 성능과도 구분한다.

| 검색 개수 | 상위 검색 결과의 완전 회수 | 운영 문맥 구성 후 완전 회수 |
| --- | --- | --- |
| 3 | 16/24 | 20/24 |
| 5 | 20/24 | 21/24 |
| 8 | 21/24 | 22/24 |
...
검색 개수 8·10은 알려진 질문에서 추가 근거를 회수했지만, 이 결과만으로 전체 생성 품질과 비용의 개선을 확인한 것은 아니다. 현재 운영값 5를 유지하고 이 설정의 결과를 회귀 기준으로 고정했다. 새 설정을 선택할 때는 문맥 길이와 생성 품질을 함께 평가한다. 기준 결과는 자동 승격하지 않으며, 검토한 후보 파일의 해시를 지정해 별도 기준으로 고정한다.

질의 재작성은 저장된 실제 전후 질의 6쌍을 같은 현재 검색·문맥 설정에서 재생했다. 원문 근거 완전 회수는 실행 기록 기준 3건 개선·1건 유지·2건 악화였다. 개선된 세 기록은 같은 질의 쌍이므로 중복을 제외하면 4쌍 중 개선 1·유지 1·악화 2다. 기존 조건 진단 질문을 사용한 관측 사례로 일반적인 성공률을 추정하지 않는다. 재작성 뒤에도 근거 충분성·인용 검증을 다시 수행하는 이유이며, 두 번째 시도까지 핵심 근거를 확보하지 못하면 종료한다. 측정 방법과 원시 결과는 [검색 회귀와 재작성 검증](retrieval-regression.md)에 정리했다.

### 5.5 기업 문서 환경과 추출 품질의 후속 검증

기업 문서의 후속 검증은 문서 검토 품질과 KV 구현 성능으로 나눈다. 문서 검토 실험은 공개 RFP·계약·사업 문서 최대 3개·60페이지에 선택용 8문항과 미사용 검증용 8문항을 구성하고, 요구사항 회수·필수 조항 누락·근거 연결·미확인 응답·시간을 비교한다. 정답은 두 검토자가 근거 페이지와 함께 작성하고, 질문당 생성 결과를 3회 확인한다. 후보는 기준보다 누락과 근거 없는 확정 답변을 늘리지 않고 회수율과 근거 정확도를 유지해야 한다.

KV 실험은 공통으로 지원되는 모델과 장비에서 기준 KV, KIVI, InfiniGen을 비교한다. 같은 문서·질문·문맥·부하에서 품질을 먼저 확인하고 첫 토큰 지연, 전체 생성 시간, GPU·CPU 메모리와 실패 건수를 측정한다. 현재 외부 모델 API 실행 시간으로 두 KV 구현의 성능을 대신 측정하지 않는다. 기업 문서 원문·정답과 공통 추론 환경은 아직 확보가 필요하며, 조건·반복 횟수·실패 처리·판정 방법은 [도메인 검증 계획](domain-validation-plan.md)에 정리했다.

현재 PDF 처리는 Vision 모델을 사용하지 않고 텍스트를 추출한다. 빈 페이지는 오류로 처리하지만, 표의 행·열 관계나 그림의 수치가 텍스트만으로 보존되는지는 별도 검수가 필요하다. 그림·표가 결론의 핵심 근거인 경우 물리 페이지의 원본과 수치·단위·장비 조건을 대조하고, 확인되지 않는 값은 미확인으로 남긴다. 향후 이미지 인식을 도입할 때에는 페이지·영역과 추출 결과를 연결하고 수치·단위를 재검증하는 절차부터 마련한다.
...
운영 중 지연을 분석하기 위해 선택적으로 LangSmith 실행 추적을 사용할 수 있다. 실행 ID 아래에 그래프 단계와 모델 생성 호출을 연결하고, 호출 슬롯 대기 시간·HTTP 소요 시간·적용된 추론 수준·제공자 사용량을 기록한다. 병렬 작업에도 부모 실행 정보를 전달해 한 실행의 흐름을 함께 볼 수 있게 했다. 문서와 모델 응답의 원문은 로컬에 보관하며 추적에는 보내지 않는다. 추적은 기본적으로 꺼져 있고, 별도 전송 작업의 실패가 보고서 생성이나 모델 재시도 정책을 바꾸지 않는다. 종료 시 전송 대기는 최대 2초이며 전송이 끝나지 않은 기록은 누락될 수 있다. 비용 통제는 기존 로컬 예산 장부를 따른다.
